...
describe('SlidingWindowController', () => {
  ...
  it('requests runtime state and toggles readonly controls for historical windows', () => {
    ...
    // Check that style element exists (CSS content may not be available in test environment)
    expect(document.querySelector('#turbo-render-sliding-window-style')).not.toBeNull();
