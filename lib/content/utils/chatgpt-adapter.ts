...
// 缓存 isTurboRenderUiNode 结果，避免重复 DOM 查询
const uiNodeCache = new WeakMap<Element, boolean>();

export function isTurboRenderUiNode(node: Element): boolean {
  const cached = uiNodeCache.get(node);
  if (cached !== undefined) {
    return cached;
  }
  const result = node.closest(TURBO_RENDER_UI_ROOT_SELECTOR) != null;
  uiNodeCache.set(node, result);
  return result;
}
...
export function scanChatPage(doc: Document = document): AdapterSnapshot {
  ...
  const debugInfo = {
    ...
    mainTagName: main?.tagName,
    mainChildren: main?.children?.length,
    selector1: main?.querySelectorAll('[data-testid^="conversation-turn-"]').length ?? 0,
    selector2: main?.querySelectorAll('[data-message-author-role]').length ?? 0,
  };
  ...
  if (candidates.length === 0) {
    ...
    console.log('[TurboRender:Debug] scanChatPage no candidates:', debugInfo);
  }
