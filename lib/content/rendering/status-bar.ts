...
import statusBarStyles from '../../styles/status-bar.css?inline';
import { injectToolbarStyles } from '../ui/base-toolbar';
import { isSupplementalHistoryEntry, resolvePreferredMessageId } from '../core/managed-history';
...
export class StatusBar {
  ...
  private mount(target: HTMLElement | null): void {
    injectToolbarStyles(this.doc, 'turbo-render-style', statusBarStyles);
    if (target == null) {
      return;
    }
  ...
  private buildEntriesKey(group: ManagedHistoryGroup, query: string, conversationId: string): string {
    ...
    const actionsKey = group.entries
      .map((entry) => {
        ...
        return [
          entry.id,
          availability?.copy ?? (isSupplementalHistoryEntry(entry) ? 'unavailable' : entry.role === 'assistant' ? 'local-fallback' : 'unavailable'),
          availability?.like ?? 'unavailable',
          availability?.dislike ?? 'unavailable',
          availability?.share ?? 'unavailable',
          availability?.more ?? (isSupplementalHistoryEntry(entry) || entry.role !== 'assistant' ? 'unavailable' : 'local-fallback'),
        ].join(':');
      })
      .join('|');
  ...
  private prepareActionTemplateButtons(
    ...
    const availability = this.currentState?.entryActionAvailability[entryKey];
    const getActionMode = (action: ArchiveEntryAction): EntryActionAvailability['copy'] => {
      if (isSupplementalHistoryEntry(entry)) return 'unavailable';
      if (entry.role !== 'assistant') return action === 'copy' ? 'local-fallback' : 'unavailable';
      return availability?.[action] ?? (action === 'copy' || action === 'more' ? 'local-fallback' : 'unavailable');
    };
    for (const action of ENTRY_ACTION_LANE[lane]) {
      if (lane === 'assistant' && (action === 'like' || action === 'dislike') && selectedAction != null && selectedAction !== action) {
  ...
  private createFallbackActionButton(
    ...
    const label = isCopiedAction ? this.t('messageActionCopied') : this.getEntryActionLabel(action, lane);
    const availability = this.currentState?.entryActionAvailability[entryKey];
    const actionMode = isSupplementalHistoryEntry(entry)
      ? 'unavailable'
      : entry.role !== 'assistant'
        ? action === 'copy' ? 'local-fallback' : 'unavailable'
        : availability?.[action] ?? (action === 'copy' || action === 'more' ? 'local-fallback' : 'unavailable');
    const title = this.getEntryActionTitle(action, label, actionMode);
  ...
  private getConversationId(): string | null {
    return this.currentState?.conversationId?.trim() || null;
  }
    ...
    const entryKey = getArchiveEntrySelectionKey(entry);
    const messageId = this.currentState?.entryHostMessageIds[entryKey] ?? resolvePreferredMessageId(entry.messageId, entry.liveTurnId, entry.turnId);
