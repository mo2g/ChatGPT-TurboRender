...
- 在对话顶部提供固定历史条，让“被处理历史”有明确入口
- 当线程长度或帧压力超过阈值时自动介入
- 在页面主世界里裁剪首屏 ` /backend-api/conversation/:id ` payload，避免超长会话完整进入官方首屏渲染路径
...
当 TurboRender 处理较早消息后，会在对话顶部插入一个固定历史条。

- 点击 `查看历史` 会展开已处理历史，并临时暂停自动重新收纳
- 点击 `展开` 可以看到 `恢复附近`、`恢复全部` 和 `暂停此会话`
- transcript 中每个分组占位块仍然可以单独恢复，但它们现在只是次入口
