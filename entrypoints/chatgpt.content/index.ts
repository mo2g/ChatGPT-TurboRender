...
const contentScript = defineContentScript({
  ...
  async main(ctx: ContentScriptContext) {
    ...
    const ensureController = (retryCount = 0) => {
      if (!isAlive() || controller != null) {
        return;
      }

      // Check if ChatGPT conversation turns are available
      const main = document.querySelector('main');
      const hasTurns = main != null && (
        main.querySelectorAll('[data-testid^="conversation-turn-"]').length > 0 ||
        main.querySelectorAll('[data-message-author-role]').length > 0
      );

      if (!hasTurns && retryCount < 30) {
        // Delay and retry - ChatGPT loads messages asynchronously
        setTimeout(() => ensureController(retryCount + 1), 500);
        return;
      }
